...
document.addEventListener('DOMContentLoaded', function () {
	...
	$('aside .filter .item .head').click(function(e) {
		...
		$(this).toggleClass('active').next().slideToggle(300, () => Sticky.refresh())
	})
	...
	// Шапка - категории
	$('header .categories > * > a.sub_link').mouseenter(function() {
		$('.overlay').fadeIn(200)
	})

	$('header .categories > *').mouseleave(function() {
		$('.overlay').fadeOut(200)
	})
...
window.addEventListener('scroll', function () {
	// Фикс. шапка
	typeof headerInit !== 'undefined' && headerInit && $(window).scrollTop() < scrollOffset && $(window).scrollTop() > headerHeight && WW < 1280
...
function updateButtonPosition() {
	let currentOffset = $(window).scrollTop() + $(window).outerHeight() - checkoutBtn.outerHeight()

	if (currentOffset >= initialOffset) {
		checkoutBtn.removeClass('fixed')
		placeholder.hide()
	} else {
		checkoutBtn.addClass('fixed')
		placeholder.show()
